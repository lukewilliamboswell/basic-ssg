...
main! : List(Str) => Try({}, [Exit(I32), ExampleErr(Str), StdoutErr(IOErr), ..])
main! = |args|
	match args.drop_first(1) {
		[source_path_arg] => print_title!(Path.unix(source_path_arg))
...
first_h1 = |html| {
	after_open = Str.split_on(html, "<h1>").get(1) ? |_| MissingHeading
	heading = Str.split_on(after_open, "</h1>").get(0) ? |_| MissingHeading
	Ok(heading)
}

## first_h1 returns the text inside the first h1 element.
expect {
	first_h1("<h1>Fruit</h1><p>Body</p>")? == "Fruit"
}

## first_h1 reports MissingHeading when there is no h1 element.
expect {
	match first_h1("<p>Body</p>") {
		Ok(_) => 1 == 0
		Err(MissingHeading) => 1 == 1
	}
}
