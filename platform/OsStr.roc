...
## UTF-8 inspection identifies its representation and escapes control characters.
expect Str.inspect(OsStr.utf8("a\nb")) == "OsStr.utf8(\"a\\nb\")"

## Unix text inspection uses the text constructor when the bytes are valid UTF-8.
expect Str.inspect(OsStr.unix("abc")) == "OsStr.unix(\"abc\")"

## Unix raw-byte inspection preserves invalid UTF-8 bytes.
expect Str.inspect(OsStr.unix_bytes([97, 255, 98])) == "OsStr.unix_bytes([97, 255, 98])"

## Windows text inspection uses the text constructor when the units are valid UTF-16.
...
## Equality distinguishes otherwise equivalent text stored in different representations.
expect OsStr.utf8("abc") != OsStr.unix("abc")

## Hashing is consistent with exact representation equality.
